...
import { deleteAuthUserAsync, updateAuthUserTiers } from '@/lib/auth-users';
import { getSupabaseAdmin } from '@/lib/db/supabase';
import bcrypt from 'bcryptjs';

// 取得所有會員清單 (100% 直通真實雲端 Supabase 資料庫，嚴禁本機檔案 Fallback)
export async function GET(request: Request) {
  try {
    ...
    if (!supabase) {
      console.error('[CRITICAL] 管理後台讀取失敗：Supabase 雲端資料庫未配置！');
      return NextResponse.json(
        {
          success: false,
          error: '雲端資料庫未配置，無法讀取會員清單。請確認環境變數 NEXT_PUBLIC_SUPABASE_URL 與 SUPABASE_SERVICE_ROLE_KEY 已設定。',
        },
        { status: 500 }
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    const supabase = getSupabaseAdmin();
    if (!supabase) {
      console.error('[CRITICAL] 管理後台新增失敗：Supabase 雲端資料庫未配置！');
      return NextResponse.json(
        {
          success: false,
          error: '雲端資料庫未配置，無法新增會員。請確認環境變數 NEXT_PUBLIC_SUPABASE_URL 與 SUPABASE_SERVICE_ROLE_KEY 已設定。',
        },
        { status: 500 }
      );
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    // 1. 強制對雲端資料庫真實 await 執行 SQL UPDATE
    const supabase = getSupabaseAdmin();
    if (!supabase) {
      console.error('[CRITICAL] 管理後台更新失敗：Supabase 雲端資料庫未配置！');
      return NextResponse.json(
        {
          success: false,
          error: '雲端資料庫未配置，無法更新會員。請確認環境變數 NEXT_PUBLIC_SUPABASE_URL 與 SUPABASE_SERVICE_ROLE_KEY 已設定。',
        },
        { status: 500 }
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    const supabase = getSupabaseAdmin();
    if (!supabase) {
      console.error('[CRITICAL] 管理後台刪除失敗：Supabase 雲端資料庫未配置！');
      return NextResponse.json(
        {
          success: false,
          error: '雲端資料庫未配置，無法刪除會員。請確認環境變數 NEXT_PUBLIC_SUPABASE_URL 與 SUPABASE_SERVICE_ROLE_KEY 已設定。',
        },
        { status: 500 }
      );
    }
